...
REPORT_TITLE = "Medical Report Analyzer using NLP & Machine Learning"
...
def write_pdf(path: Path) -> None:
    ...
    story = [Paragraph(REPORT_TITLE, styles["Title"]), Spacer(1, 18)]
    for title, paragraphs in SECTIONS:
...
def write_basic_pdf(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    text_lines = [REPORT_TITLE, ""]
    for title, paragraphs in SECTIONS:
    ...
    escaped = []
    for line in text_lines[:90]:
        line = line.replace("\\", "\\\\").replace("(", "\\(").replace(")", "\\)")
        escaped.append(line[:95])

    content = ["BT", "/F1 9 Tf", "45 760 Td", "12 TL"]
    for line in escaped:
...
def main() -> None:
    write_markdown(Path("docs/technical_report.md"))
    write_pdf(Path("reports/technical_report.pdf"))
    print("Generated docs/technical_report.md and reports/technical_report.pdf")
